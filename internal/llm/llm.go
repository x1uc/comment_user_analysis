...
var (
	test_prompt string = `# Role
你是一位精通中国互联网舆论生态、法学辩论逻辑以及中文网络俚语（包括反讽、隐喻）的社会舆论分析师。
...
type LLMClient interface {
	GetCommentLevel() models.AiResponse
}
...
func (c *DeepSeek) GetCommentLevel() (*models.AiResponse, error) {
	c.Api_key = "sk-f609b38bf28b41f5882755ec7f5b400f"
	client := openai.NewClient(
		option.WithBaseURL("https://api.deepseek.com"),
		option.WithAPIKey(c.Api_key), // defaults to os.LookupEnv("OPENAI_API_KEY")
	)
	chatCompletion, err := client.Chat.Completions.New(context.TODO(), openai.ChatCompletionNewParams{
		// 1. 必须用 deepseek-chat，因为 reasoner 不支持 JSON 模式
		Model: openai.ChatModel("deepseek-reasoner"),

		Messages: []openai.ChatCompletionMessageParamUnion{
			openai.SystemMessage(test_prompt),
			openai.UserMessage("非常正确，大家应该首先了解我国的《禁毒法》和毒品管控法律体系，且要知道我国禁毒工作一直很严。涉毒人员登记和治安处罚记录是两码事，不是封存治安处罚记录就意味着对毒品管控和打击减弱。"),
		},

		// 2. 这里的写法对应您查到的 ChatCompletionNewParamsResponseFormatUnion
